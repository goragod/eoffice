        ...
        if ($request->initSession() && $request->isReferer() && $login = Login::isMember()) {
            if (Login::notDemoMode($login) && preg_match('/download_([0-9]+)/', $request->post('id')->toString(), $match)) {
                ...
                $result = $this->db()->createQuery()
                    ->from('edocument E')
                    ->join('edocument_download D', 'LEFT', array(array('D.id', 'E.id'), array('D.member_id', (int) $login['id'])))
                    ->where(array('E.id', (int) $match[1]))
                    ->groupBy('E.id')
                    ->first('E.id', 'E.topic', 'D.id download_id', 'D.downloads', 'E.file', 'E.ext', 'E.size');
                if ($result) {
                    // ไฟล์
