    ...
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        if (!$error) {
            ...
            try {
                ...
                if (!$db->fieldExists($table, 'urgency')) {
                    $db->query("ALTER TABLE `$table` ADD `urgency` TINYINT(1) NOT NULL DEFAULT 2");
                }
                $content[] = '<li class="correct">ปรับปรุงตาราง `'.$table.'` สำเร็จ</li>';
                // ตาราง inventory
                ...
                if (!$db->tableExists($table)) {
                    $db->query("CREATE TABLE `$table` (`inventory_id` int(11) NOT NULL,`name` varchar(20) NOT NULL,`value` varchar(150) NOT NULL) ENGINE=MyISAM DEFAULT CHARSET=utf8");
